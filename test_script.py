...
if __name__ == "__main__":
    ...
    # (freq, mono_RCS) = RCS_vs_freq(radius, ratio, background, sphere_material=pec, \
    #                         sensor_location=observer , save_file = 'PEC', show_plot = 0)
    # plotOneMonoRCS(radius, sphere_material, background, mono_RCS, ratio = ratio, \
    #                 savefile = "output/mono_rcs")
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        Bistatic_RCS(rad, freq_bis, background_material=vacuum, sphere_material=pec, distance=observer, phi=angle, save_filename="output/bistatic_rcs.png")
    print("Finished PEC Bistatic: Plot Bistatic RCS for Perfect Electric Conductor Sphere as Perfect_Electric_Conductor_Sphere.png\n")



    # with warnings.catch_warnings():
    #     warnings.simplefilter("ignore")
    #     Bistatic_RCS(rad, freq, background_material=vacuum, sphere_material=pec, distance=observer, phi=angle, save_filename="output/Perfect_Electric_Conductor_Sphere_Pi_2")
    # print("Finished PEC Bistatic: Plot Bistatic RCS for Perfect Electric Conductor Sphere as Perfect_Electric_Conductor_Sphere_Pi_2.png\n")

    #PART 2: PLOT ONE MONOSTATIC RCS FOR PERFECT DIELECTRIC
    # with warnings.catch_warnings():
    #     warnings.simplefilter("ignore")
    #     RCS_vs_freq(radius = 0.5, ratio = np.arange(0.01,1.61,0.01), \
    #         background_material = DielectricMaterial(1,0), sphere_material = pec, \
